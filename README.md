...
- **Сжатие истории экономит токены.** Окно освобождается, стоимость запроса снижается — диалог продолжается вместо переполнения. В демо: ~1.4k ток в окне со сжатием против ~11.5k без, расход API — ₽0.26 против ₽1.79.
- **Сводка оставляет более сухие факты.** Суть диалога сохраняется (жалоба, точки, техника, ограничения), но часть подробностей и деталей теряется — нюансы после сжатия лучше уточнять заново.
